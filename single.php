...
Use: Single article for ZMAN-NEWS
10/19/2022
...
<body>
    ...
    <?php
        ...
        global $db;
        $art = filter_input(INPUT_GET, 'varname');
    ?>

    <?php
        $query = sprintf('SELECT title FROM articles WHERE id=%d',$art);
        $statement = $db->prepare($query);
        $statement->execute();
        $title = $statement->fetchAll();
        $statement->closeCursor();
        
        $query = sprintf('SELECT img FROM articles WHERE id=%d',$art);
        $statement = $db->prepare($query);
        $statement->execute();
        $image = $statement->fetchAll();
        $statement->closeCursor();

        $query = sprintf('SELECT news FROM articles WHERE id=%d',$art);
        $statement = $db->prepare($query);
        $statement->execute();
        $news = $statement->fetchAll();
        $statement->closeCursor();

        $query = sprintf('SELECT author FROM articles WHERE id=%d',$art);
        $statement = $db->prepare($query);
        $statement->execute();
                    ...
                    <div class="position-relative mb-3">
                        <img class="img-fluid w-100" src="
                                    <?php foreach ($image as $img): ?>
                                        <?php echo $img['img']; ?>
                                    <?php endforeach;?>
                        " width="700" height="435" style="object-fit: cover;">
                        <div class="overlay position-relative bg-light">
                        <div class="mb-3">
                                <span class="px-1">Written by: 
                            ...
                            <div>
                                ...
                                <h6>Enter password to wipe this article</h6>
                                    <form name="form" action="article-delete.php" method="GET"> 
                                    <!--<input type="file" name="img" id="fileToUpload"> -->
                                    <input type="text" name="pass" id="pass" placeholder="enter password">
                                    <input type="hidden" name="number" value =<?php echo $art ?>>
    ...
    <!-- Back to Top -->
    <a href="#" class="btn btn-dark back-to-top" style="display: inline;"><i class="fa fa-angle-up"></i></a>
